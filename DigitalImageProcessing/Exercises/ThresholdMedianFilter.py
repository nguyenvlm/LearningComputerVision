...
def thresholdMedianFilter(image, threshold=5, size=3):
    ...
    if threshold not in range(256):
        raise ValueError("Improper threshold value! (must be in range [0, 256])")
...
if __name__ == "__main__":
    ...
    noise[[np.random.randint(0, i - 1, int(num_pepper)) for i in img.shape]] = 0
    img = noise
    ...
    for size in (3, 5, 7):
        for t in range(5, 31, 5):
            cv2.imwrite("%s\lena_color_256_thres%d_size%d.tif"%(outputDir, t, size), thresholdMedianFilter(image=img, threshold=t, size=size))